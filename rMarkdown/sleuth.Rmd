...
```{bash kallisto_command, eval=FALSE, include=FALSE}
#!/bin/bash
...
# Sleuth	provides	another	test	called	Wald	test,	which	returns	a	‘b’	value	that “it	is	analogous	to,	but	not	equivalent to,	the	foldchange.”
...
plot_ma(so, "conditionHFD", test_type = "wt", which_model = "full", sig_level = 0.01, point_alpha = 0.2, sig_color = "red", highlight = NULL, highlight_color = "green")
```

```{r volcano_plot, echo=TRUE}
plot_volcano(so, "conditionHFD", test_type = "wt", which_model = "full", sig_level = 0.01, point_alpha = 0.2, sig_color = "red", highlight = NULL)
```
