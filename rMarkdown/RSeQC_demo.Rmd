...
## rRNA percentage
Ribosomal RNA (rRNA) often exceeding 90% of the total RNA. Prior library treatment of rRNA removal should result in less than 5% of the reads mapped to rRNA sequences.
...
prefix_bam="prefix your output BAM file"
...
subset_dist <- data.frame(SM4998_distribution[13:15, c(1, 5)])
...
piePlot1 + geom_bar(width = 1, stat = "identity") +
  ...
  blank_theme +
  geom_text(aes(
    y = Per_Assigned_Tag / 3 + c(0, cumsum(Per_Assigned_Tag)[-length(Per_Assigned_Tag)]),
...
```{r echo=FALSE}
cat(readLines("C:/Users/lingzili/Documents/seq_tutorials/data/SM4998.distribution.output.txt"), sep = "
")
```

## Gene body coverage

Calculate the RNA-seq reads coverage over gene body. Uniform distributed reads across genebodies is a prerequisite for accurate quantification of gene expression!

```{bash eval=FALSE}
locate geneBody_coverage
geneBody_coverage="your file path to geneBody_coverage.py"

housekeepBED="your file path to hg38.HouseKeepingGenes.bed"
...
```{r geneBodyCoverage, echo=FALSE, eval=TRUE, out.width = "400px"}
